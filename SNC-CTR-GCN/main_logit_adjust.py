...
if os.name != 'nt':  # 非Windows系统
    ...
    sys.modules['resource'] = DummyResource()
def init_seed(seed):
...
class LogitAdjust(nn.Module):
    ...
    def __init__(self, cls_num_list,output_device ,tau=1, weight=None):
        super(LogitAdjust, self).__init__()
        self.output_device = output_device
        cls_num_list = torch.FloatTensor(cls_num_list).to(self.output_device)
...
class Processor():
    ...
    def load_model(self):
        ...
        print(self.model)
        self.loss = LogitAdjust(self.cls_num_list,output_device=self.output_device).cuda(output_device)
...
def shot_acc(preds, labels, train_data, many_shot_thr=115, low_shot_thr=90, acc_per_cls=False):
